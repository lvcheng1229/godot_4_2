...
bool HoudiniInputTranslator::upload_input_data(HoudiniInput *houdini_input) {
	...
	if (input_node_id < 0) {
		HOUDINI_CHECK_ERROR_RETURN(HoudiniEngineUtils::create_node(-1, String("SOP/merge"), true, &input_node_id), false);
	...
	for (int index = 0; index < input_node_array.size(); index++)
	{
		...
		HOUDINI_CHECK_ERROR_RETURN(HoudiniApi::connect_node_input(HoudiniEngine::get().get_session(), input_node_id, input_index++, current_node_id, 0), false);
	}
...
bool HoudiniInputTranslator::connect_input_node(HoudiniInput *houdini_input) {
	...
	HOUDINI_CHECK_ERROR_RETURN(HoudiniApi::connect_node_input(HoudiniEngine::get().get_session(), hda_node_id, houdini_input->get_sop_input_index(), input_node_id, 0), false);
	return true;
}
